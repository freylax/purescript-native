...
moduleToIL (Module _ coms mn _ imps _ foreigns decls) env = do
  ...
  where
    ...
    annotValue (AST.Function _ (AST.TypeSpecSeq AST.Auto Nothing) (Just name)
                [] (AST.Block _ [AST.Return _ ret])) | isLiteral ret = (name, True)
    annotValue (AST.Function _ (AST.TypeSpecSeq AST.Auto Nothing) (Just name) [] _) = (name, False)
    ...
    nonRecToIL dt a i e@(extractAnn -> (_, com, _, _))
      | not (null com) = do -- withoutComment <- asks optionsNoComments
    ...
    nonRecToIL ModuleDecl (ss, _, _, _) ident val = do
      il <- valueToIL val
      pure $ AST.Function Nothing (AST.TypeSpecSeq AST.Auto Nothing)
        (Just $ identToIL ident) [] (AST.Block Nothing [AST.Return Nothing il])
    nonRecToIL RecLetDecl (ss, _, _, _) ident val = do
      ...
      pure $
        ...
        in
          case il of
            AST.Function _ ret name args (AST.Block Nothing ils) ->
    ...
    valueToIL (ObjectUpdate _ o ps) = do
      ...
      return $ AST.App Nothing (AST.Function Nothing (AST.TypeSpecSeq AST.Auto Nothing) Nothing
                                [] (AST.Block Nothing sts)) []
    valueToIL (Abs _ arg val) = do
      ret <- valueToIL val
      let ilArg = case arg of
                    UnusedIdent -> []
                    _           -> [((AST.TypeSpecSeq AST.Auto Nothing),identToIL arg)]
      return $ AST.Function Nothing (AST.TypeSpecSeq AST.Auto Nothing) Nothing ilArg
        (AST.Block Nothing [AST.Return Nothing ret])
    valueToIL e@App{} = do
      let (f, args) = unApp e []
    ...
    valueToIL (Constructor _ _ (ProperName ctor) fields) =
      let body = AST.ObjectLiteral Nothing $ (mkString ctor, AST.BooleanLiteral Nothing True) :
          ...
          fn = foldr (\f inner -> AST.Function Nothing (AST.TypeSpecSeq AST.Auto Nothing) Nothing
                       [((AST.TypeSpecSeq AST.Auto Nothing),identToIL f)]
                       (AST.Block Nothing [AST.Return Nothing inner])) body fields
      in return fn


    iife :: Text -> [AST] -> AST
    iife v exprs = AST.App Nothing (AST.Function Nothing (AST.TypeSpecSeq AST.Auto Nothing) Nothing []
                                    (AST.Block Nothing $ exprs ++ [AST.Return Nothing $ AST.Var Nothing v])) []
    ...
    qualifiedToIL f (Qualified (Just (ModuleName [ProperName mn'])) a)
      | mn' == C.prim = AST.Var Nothing . identToIL $ f a
    ...
    qualifiedToIL f (Qualified (Just mn') a) =
      AST.Indexer Nothing (AST.Var Nothing . identToIL $ f a) (AST.Var Nothing (moduleNameToIL mn'))
    qualifiedToIL f (Qualified _ a) =
      AST.Indexer Nothing (AST.Var Nothing . identToIL $ f a) (AST.Var Nothing "")
      ...
      return $ AST.App Nothing (AST.Function Nothing (AST.TypeSpecSeq AST.Auto Nothing) Nothing []
                                (AST.Block Nothing
                                  (assignments ++ concat ils ++
                                    [AST.Throw Nothing (AST.StringLiteral Nothing $ mkString failedPatternMessage)])))
        []
      where
    ...
    binderToIL varName done (ConstructorBinder
      ...
      return $ case ctorType of
        ProductType -> il
        SumType ->
          [AST.IfElse Nothing (AST.InstanceOf Nothing (AST.Var Nothing varName)
      ...
      where
        ...
        go ((field, binder) : remain) done' = do
          ...
          return (AST.VariableIntroduction Nothing argVar
                  (Just $ accessorString (mkString $ identToIL field) $ AST.Var Nothing varName) : il)
    ...
    literalToBinderIL varName done (NumericLiteral num@Left{}) =
      return [AST.IfElse Nothing
               (AST.Binary Nothing AST.EqualTo (unbox' int $ AST.Var Nothing varName)
                 (AST.NumericLiteral Nothing num)) (AST.Block Nothing done) Nothing]
    ...
    literalToBinderIL varName done (CharLiteral c) =
      return [AST.IfElse Nothing
               (AST.Binary Nothing AST.EqualTo
                 (unbox' string $ AST.Var Nothing varName)
                 (AST.StringLiteral Nothing (fromString [c]))) (AST.Block Nothing done) Nothing]
    literalToBinderIL varName done (StringLiteral str) =
      return [AST.IfElse Nothing
              (AST.Binary Nothing AST.EqualTo (unbox' string $ AST.Var Nothing varName)
               (AST.StringLiteral Nothing str)) (AST.Block Nothing done) Nothing]
    literalToBinderIL varName done (BooleanLiteral True) =
      return [AST.IfElse Nothing (unbox' bool $ AST.Var Nothing varName)
               (AST.Block Nothing done) Nothing]
    literalToBinderIL varName done (BooleanLiteral False) =
      return [AST.IfElse Nothing
               (AST.Unary Nothing AST.Not (unbox' bool $ AST.Var Nothing varName)) (AST.Block Nothing done) Nothing]
    ...
    literalToBinderIL varName done (ArrayLiteral bs) = do
      ...
      where
        ...
        go done' index (binder:bs') = do
          ...
          return (AST.VariableIntroduction Nothing elVar
                  (Just (AST.Indexer Nothing (AST.NumericLiteral Nothing (Left index))
                         (AST.Var Nothing varName))) : il)
