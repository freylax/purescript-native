...
data AST
  ...
  -- ^ An object literal
  | Function (Maybe SourceSpan) TypeSpecSeq (Maybe Text) [(TypeSpecSeq,Text)] AST
...
withSourceSpan withSpan = go where
  ...
  go (Function _ ret name args j) = Function ss ret name args j
...
everywhere f = go where
  ...
  go (Function ss ret name args j) = f (Function ss ret name args (go j))
...
everywhereTopDownM f = f >=> go where
  ...
  go (Function ss ret name args j) = Function ss ret name args <$> f' j
...
everything (<>.) f = go where
  ...
  go j@(Function _ _ _ _ j1) = f j <>. go j1
